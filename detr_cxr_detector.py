...
def train(args) -> None:
    ...
    start_epoch = 0
    best_map = -1.0
    resume_path = resolve_resume_path(args, "detr_last.pt")
    ...
    for epoch in range(start_epoch, args.epochs):
        ...
        for epoch_step, (images, targets) in enumerate(train_loader):
            ...
            if args.checkpoint_every_steps and step % args.checkpoint_every_steps == 0:
                save_training_checkpoint(args, model, opt, scaler, epoch, step, best_map, "detr_last.pt", ema)
        ...
        save_training_checkpoint(args, model, opt, scaler, epoch + 1, step, best_map, "detr_last.pt", ema)
        if args.save_every_epoch:
            save_training_checkpoint(args, model, opt, scaler, epoch + 1, step, best_map, f"detr_epoch_{epoch + 1:03d}.pt", ema)
...
def parse_args():
    ...
    if args.smoke:
        ...
        args.base_channels = 8; args.hidden_dim = 32; args.num_queries = 8; args.num_decoder_layers = 1; args.max_steps = 1; args.steps_per_epoch = 1; args.warmup_steps = 1; args.log_every = 1; args.positive_only = True
    return args
